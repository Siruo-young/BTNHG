...
class BTNHGV2ParameterClass():
	...
	lr=0.005 #学习率
	useLrScheduler=True #是否使用学习率调度器
	...
	num_heads=4	#头数
	num_layers=3	#层数
	HGT_useProj=True	#是否使用投影

	########### 数据集
	train_size=0.8	#数据划分比例
	shuffle=True	#是否打乱数据
	batch_size=4096	#批量大小
	randSeed=42	#随机种子
	resetSeed=False	#是否重置随机种子

	########### 存储
	dataPath=r"E:\PycharmProjects\BTNHG\archive"
	...
	cnn_hidden_channels=1 #recommended 1 #隐藏层通道数
	cnn_out_channels=1 #recommended 1 #输出通道数
	# cnn_hidden_fc_out=4 #recommended 4

	cnn_kernel_height=3 #卷积核高度
	cnn_kernel_width=3 #卷积核宽度
	
	pool_height=2 #池化高度
	pool_width=1 #池化宽度

	#########transformer
	tf_dim_feedforward=32 #减少，准确率会下降。增加，好像没什么变化。 feedforward维度
	tf_num_heads=1 #head增加，准确率反而下降。 head数
	tf_num_layers=1 #layer增加，准确率未见提升，甚至会下降。 layer数